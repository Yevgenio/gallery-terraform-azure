...
**SSH access** (`bastion.tf`) — Azure Bastion (Standard SKU, `tunneling_enabled = true`). VMs have no public IPs; all SSH goes through Bastion. Use `az network bastion ssh` from the CLI.
